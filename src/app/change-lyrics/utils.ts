...
export function getDistinctChangedWords(lyrics: LyricLine[]): string[] {
    ...
    lyrics.forEach(line => {
        if (!line.original || line.original === line.modified) return;

        const diffs = diffWords(line.original, line.modified);

        diffs.forEach(part => {
            if (part.removed) {
                part.value.trim().split(/\s+/).filter(Boolean).forEach(word => {
...
export function handleReplaceAll(
    ...
    setLyrics(prevLyrics => {
        const updatedLyrics = prevLyrics.map(line => {
            ...
            if (exactPattern) {
                newModified = newModified.replace(exactPattern, () => replaceWith + termPunct);
            }
            ...
            // Count removed original words (consistent with getDistinctChangedWords)
            const diffs = diffWords(line.original, newModified);
            diffs.forEach(part => {
                if (part.removed) {
                    part.value.trim().split(/\s+/).filter(Boolean).forEach(word => {
                        const normalized = word
                            .replace(/^[.,!?;:"'\[\]{}\(\)\-—_]+|[.,!?;:"'\[\]{}\(\)\-—_]+$/g, '')
                            .toLowerCase();
                        if (normalized) totalReplacements++;
                    });
                }
            });
