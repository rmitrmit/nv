...
function OrderReviewSamplePageContent() {
    ...
    const lyrics = useMemo(() => {
        return lyricsData.filter(line => line.original !== line.modified);
    }, [lyricsData]);
    ...
    const generateDownloadContent = (): string => {
        ...
        const content = [
            ...
            `URL: ${songUrl || 'N/A'}`,
            ``,
            `LYRIC CHANGES (${distinctChangedWords.length} words modified):`,
    ...
    const handleSampleRequest = async () => {
        ...
        if (lyrics.filter(line => line.modified !== line.original).length === 0) {
            toast.error("No lyric changes detected", {
                                ...
                                <div className="flex flex-col space-y-2 overflow-y-auto md:h-auto lg:h-full">
                                    ...
                                    <div className="space-y-2 my-3">
                                        <div className="pb-8 pt-4 px-4 bg-white">
                                            <h4 className="text-lg font-medium text-blue-800">Lyric Changes ({distinctChangedWords.length} word{distinctChangedWords.length > 1 ? 's' : ''})</h4>
